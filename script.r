# Read data from the results.dat storing it a Variable
Data <- read.table("results.dat", header = TRUE)
...
# Time Variable used to store ow long the process ran for
Time <- 10
...
CPUBusy <- function(cpuIdle)
{
	...
	# Need to divide by 100 to get from percentage to decimal so Ui is less than or equal to 1.0
